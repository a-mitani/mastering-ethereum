...
Genesisファイルとは、ネットワークでやり取りされるブロックチェーンの最初（Block番号 "0"）のブロックである「Genesisブロック」の情報を記述したファイルです。プライベート・ネットで独自のブロックチェーンをやり取りしていくため、このGenesisブロックとして独自に定義した、独自のGenesisファイルを用意して利用します[^2]。

Frontierのリリース以降はGethのデフォルトの動作としてFrontierの

ブロックチェーン革命の節で説明したとおり
...
次に上記ディレクトリ内にプライベート・ネット用の独自のGenesisファイルを作成し配置します。
...
[^2] 独自のgenesisファイルを使用することの最大の目的は採掘を容易にすることです。独自のgenesisファイルを用意することなく、デフォルトのgenesisブロックからプライベート・ネットのブロックチェーンをやり取りすることも可能です。しかし、デフォルトのgenesisブロック（＝[Frontierのgenesisブロック](http://etherscan.io/block/0)）の採掘難易度（Difficulty）が非常に高く（17,179,869,184）設定されたため、プライベート・ネットでも採掘が非常に困難になりました。そのためDifficultyの小さい独自のGenesisファイルを用意してプライベート・ネット内での採掘を用意することが慣習になっています。
