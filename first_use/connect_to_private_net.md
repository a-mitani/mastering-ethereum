...
（悪意あるノードも含む）不特定多数のノードが参加するライブ・ネットに対して、自分または自分の知るノードのみが参加するEthereumネットワークをプライベート・ネットと呼びます。限られた参加者のみのネットワークなので、容易にEtherが採掘が可能ですし[^1]、安全性も高いネットワークになります。そのため、Ethereumの動作を調べたり、分散型アプリケーション（Dapp）の開発作業など個人的な作業を行うには、プライベート・ネットを立ち上げてそこでいろいろ弄ってみると便利です。
...
事前に任意の場所にプライベート・ネット用の各種データを格納するディレクトリを作成します[^2]。データ用のディレクトリを準備します。
...
###脚注
[^2] ただし、テスト・ネットで採掘したEtherはテスト・ネット内のみで有効で、ライブ・ネットでは使用できないことに注意してください。
