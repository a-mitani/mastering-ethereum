...
* **Expressionsタグ**：`{{xxx}}`の構文で用います。すぐ後に後述するテンプレートヘルパーのメソッドの戻り値に置き換わる働きをします。今回の client/main.html の例では`{{isMining}}`は client/main.js の`isMining`メソッドの返り値に置き換わることになります。
