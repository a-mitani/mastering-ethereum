...
###Ethereumとは
[Ethereum](https://www.ethereum.org/)は、上述のようなブロックチェーンを利用した分散アプリケーションを開発しサービス提供を行う際の障壁を取り除くことを目的とした「分散アプリケーションプラットフォーム」です。そのプラットフォームを形作るためのプロトコル定義や実装がオープンソース・プロジェクトとしてETHDEVチームとグローバルな有志により行われています。現在βリリース（Frontier）にあり、近々にも正式リリースが成される予定です。
