...
Ethereumでは各アカウントの状態は「トランザクション」を起点にして遷移していきます [^1]。EOAがトランザクションを生成しそれをEthereumネットワーク上に送信します。採掘者は受信したトランザクションの正当性をチェックし、問題なければトランザクションとトランザクションの内容に基づいて変化した最新のアカウントの状態をブロックチェーンに埋め込みます。
