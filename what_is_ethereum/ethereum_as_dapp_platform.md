...
一方で、Contract は、EOAからトランザクションを介して生成されます。Contractは一種の自動エージェントであり、EOAが発信するトランザクションをトリガに、コントラクト・コードを実行します。
...
Ethereumでは各アカウントの状態は「トランザクション」により遷移していきます [^3]。EOAがトランザクションを生成しそれをEthereumネットワーク上に送信します。採掘者は受信したトランザクションの正当性をチェックし問題なければ、そのトランザクションの情報とトランザクションの内容に基づいて変化した最新のアカウントの状態をブロックチェーンに埋め込みます。<!--[TODO]脚注で、ビットコインの場合とは異なり、状態もブロックチェーンに組み込まれることを書く。また、詳細は「Ethereumの内部の章に書くことも明記。-->
