...
Ethereumでは各アカウントの状態は「トランザクション」により遷移していきます [^3]。EOAがトランザクションを生成しそれをEthereumネットワーク上に送信します。採掘者は受信したトランザクションの正当性をチェックし問題なければ、そのトランザクションの情報とトランザクションの内容に基づいて変化した最新のアカウントの状態をブロックチェーンに埋め込みます。<!--脚注で、ビットコインの場合とは異なり、状態もブロックチェーンに組み込まれることを書く。また、詳細は「Ethereumｎ内部の章に書くことも明記。-->
