...
「任意のデータ」には、分散アプリケーションを動作させるための入力データが格納されます。例としてここで、IPアドレスとドメイン名の紐づけを管理するドメイン管理用の分散アプリケーションを考えてみます。ドメイン名を登録したいユーザーはEOAから分散アプリケーションのCAに対するトランザクションを生成します。そのトランザクションには、紐づけたいIPアドレスとドメイン名の情報を含めます。トランザクションを受信した採掘者はトランザクション内のデータを基に、
