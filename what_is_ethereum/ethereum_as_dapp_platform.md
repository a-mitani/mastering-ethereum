...
「任意のデータ」には、分散アプリケーションを動作させるための入力データが格納されます。例えばここで、IPアドレスとドメイン名の紐づけを管理するドメイン管理用の分散アプリケーションを考えてみます。ドメイン名を登録したいユーザーはEOAから、その分散アプリケーションのCAに対するトランザクションを生成します。そのトランザクションには、紐づけたいIPアドレスとドメイン名の情報をこの「任意のデータ」のフィールドに含めます。トランザクションを受信した採掘者はトランザクション内のデータを基に、CAのコードを実行し、もし今回のトランザクションに含まれたIPアドレスとドメイン名が登録されていなければ、新しくこの紐づけ情報をCAの`storage`フィールドに格納します。
